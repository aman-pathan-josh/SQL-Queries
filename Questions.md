### Sample Databse used: Northwind (https://docs.yugabyte.com/preview/sample-data/northwind/)


#### 1)  Find the number of orders sent by each shipper.

```sql
SELECT ship_via AS ShipperId, COUNT(ship_via) AS TotalOrders
FROM orders
GROUP BY ship_via;
```

#### 2)  Find the number of orders sent by each employee

```sql
SELECT employee_id AS EmployeeId, COUNT(*) AS TotalOrders
FROM orders
...
```sql
SELECT customers.company_name AS CompanyName, orders.order_id AS OrderId, 
	SUM(order_details.unit_price * order_details.quantity * (1-order_details.discount)) AS TotalPrice
FROM orders
...
```sql
SELECT orders.order_id AS Order_ID, customers.company_name AS CompanyName, 
CONCAT(employees.first_name,' ', employees.last_name) AS EmployeeName
FROM orders
INNER JOIN customers ON orders.customer_id = customers.customer_id
INNER JOIN employees ON orders.employee_id = employees.employee_id
WHERE orders.order_date > '1998-01-01' AND orders.shipped_date > orders.required_date
GROUP BY orders.order_id , customers.company_name,employees.first_name,employees.last_name
